  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
        <h1 className="font-headline font-bold text-2xl mb-4">Product Not Found</h1>
        <button 
          onClick={() => router.push('/')}
      ...
      <main className="bg-surface-container-lowest rounded-t-[30px] -mt-16 relative z-20 flex-grow px-6 pt-8 pb-32 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
        ...
        <div className="mb-8">
          ...
          <p className="text-on-surface-variant leading-relaxed text-sm">
            {product.description.split('seasoned beef').map((part, i, arr) => (
              <React.Fragment key={i}>
                {part}
                {i < arr.length - 1 && <span className="italic font-semibold text-secondary">seasoned beef</span>}
              </React.Fragment>
            ))}
          </p>
        </div>
