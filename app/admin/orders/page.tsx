...
export default function AdminOrders() {
  ...
  const fetchOrders = async () => {
    const url = filter === 'all' ? 'http://localhost:8000/orders' : `http://localhost:8000/orders?status=${filter}`;
    try {
          ...
          {orders.map(order => (
            ...
            >
              ...
              <div className="text-right">
                <p className="font-label font-bold text-lg">₦{order.total.toLocaleString()}</p>
